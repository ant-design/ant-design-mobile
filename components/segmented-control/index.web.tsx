...
export default class SegmentedControl extends React.Component<SegmentedControlProps, any> {
  ...
  renderSegmentItem(idx, value, selected) {
    const { prefixCls, enabled, tintColor } = this.props;
    ...
    return (
      <Touchable key={idx} disabled={!enabled} activeClassName={`${prefixCls}-item-active`}>
        <div className={itemCls}
          ...
          onClick={!enabled ? undefined : (e) => this.onClick(e, idx, value)}
        >
  ...
  render() {
    const { className, prefixCls, style, enabled, values = [] } = this.props;
