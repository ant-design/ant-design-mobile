...
| value    |  Selected value of `Menu`, it's a array which includes first and second level's `value`. When Menu is in mult select mode, if level is 1, all values of array mean mult select options; if level is 2, the first value of array means level 1 's option, mult select options are the rest | Array | [] |
