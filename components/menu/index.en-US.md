...
| value    |  Selected value of `Menu`, it's a array which includes first and second level's `value`. When menu is in multi select mode, if level is `1`, all values of array mean multi select options; if level is `2`, the first value of array means first level's option, multi select options are the rest | Array | [] |
