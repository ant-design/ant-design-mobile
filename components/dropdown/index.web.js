import React from 'react';
import ReactDOM from 'react-dom';
import Dialog from 'rc-dialog';
import assign from 'object-assign';
let closeFn = () => {
};
function create(instanceId, config, content, afterClose = closeFn) {
    const props = assign({}, {
        prefixCls: 'am-dropdown',
    }, config);
    const { prefixCls, transitionName, maskTransitionName, maskClosable = true } = props;
    let div = document.createElement('div');
    document.body.appendChild(div);
    let d;
    function close() {
        d.setState({
            visible: false
        });
        if (div) {
            ReactDOM.unmountComponentAtNode(div);
            div.parentNode.removeChild(div);
            div = null;
        }
        afterClose(instanceId);
    }
    ReactDOM.render(React.createElement(Dialog, {prefixCls: prefixCls, visible: true, title: "", footer: "", transitionName: transitionName || `${prefixCls}-slide-fade`, maskTransitionName: maskTransitionName || `${prefixCls}-fade`, onClose: close, maskClosable: maskClosable}, content), div, function () {
        d = this;
    });
    return {
        instanceId,
        close,
    };
}
const ins = {
    defaultInstance: null,
    instances: [],
};
let instanceId = 1;
export default class Dropdown {
}
Dropdown.newInstance = () => {
    let _i;
    return {
        show: (content, config) => {
            _i = create(instanceId++, config, content, (iId) => {
                for (let i = 0; i < ins.instances.length; i++) {
                    if (ins.instances[i].instanceId === iId) {
                        ins.instances.splice(i, 1);
                        return;
                    }
                }
            });
            ins.instances.push(_i);
        },
        hide: () => {
            _i.close();
        },
    };
};
Dropdown.show = (content, config) => {
    ins.defaultInstance = create('0', config, content, (iId) => {
        if (iId === '0') {
            ins.defaultInstance = null;
        }
    });
};
Dropdown.hide = () => {
    ins.defaultInstance.close();
};
